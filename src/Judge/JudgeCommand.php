 ...
 *
 * The judge is a single-turn scoring call, not an agent run, so the command
 * is minimal: the prompt drives it and the model is pinned from config, with
 * no stream-json, tool restriction, or turn cap. The model is passed
 * explicitly on every call so the judge model is never derived from the
