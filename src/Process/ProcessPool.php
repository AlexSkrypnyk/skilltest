 ...
 *
 * Up to `concurrency` child processes run at once, and the next queued
 * command starts the moment a slot frees. Each command carries its own
 * working directory, and its stdout is captured verbatim; stderr is discarded
 * so a chatty process cannot deadlock on an unread pipe. Every process is
 * bounded by the same wall-clock timeout: a hang is terminated (escalating to
 * an untrappable SIGKILL) and reported with the timeout exit code, so it
 * never stalls the pool. Commands run through `exec` so the tracked process
 * is the command itself, not a wrapping shell that would outlive a kill.
 * Results are returned keyed and ordered exactly as the commands were given,
 * so a caller gets the same outcome whether concurrency is 1 or 16.
 */
